...
{
	//IplImage* src=cvLoadImage(fname,1);
	//return test_detector_by_cvImage(src);
    image *src= &load_image_color(fname,0,0);
    return test_detector_by_image(src);
}

struct object_info * test_detector_by_image( image *imageptr) {
//#define TIME_TEST
//#define DRAW_TEST
    int j;
    float nms = .4;
